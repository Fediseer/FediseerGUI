...
export class MyGuaranteesComponent implements OnInit {
  ...
  public async ngOnInit(): Promise<void> {
    ...
    this.api.getCurrentInstanceInfo().subscribe(response => {
        ...
        this.guarantor = response.successResponse!.guarantor ?? '';
      }
    );
    this.api.getGuaranteesByInstance(this.authManager.currentInstanceSnapshot.name!).subscribe(response => {
      if (!response.success) {
        this.messageService.createError(`There was an api error: ${response.errorResponse!.message}`);
        return;
      }

      this.instancesGuaranteedByMe = response.successResponse!.instances;
    });
  }
